...
class WorkflowCard(QFrame):
    ...
    def paintEvent(self, event):
        ...
        c = (QColor(0, 0x9F, 0xAA, 40) if self._selected else
             QColor(0, 0x9F, 0xAA, 20) if self._hover else
             QColor(0x2a, 0x2a, 0x2a) if dark else QColor(0xf5, 0xf5, 0xf5))
        painter.setPen(Qt.NoPen)
        painter.setBrush(c)
        painter.drawRoundedRect(2, 2, w - 4, h - 4, 8, 8)

        painter.setPen(QColor(0, 0x9F, 0xAA) if self._selected else
                       QColor(0x88, 0x88, 0x88) if self._hover else
                       QColor(0x44, 0x44, 0x44))
        painter.setBrush(Qt.NoBrush)
        painter.drawRoundedRect(2, 2, w - 4, h - 4, 8, 8)

        # Thumbnail: clip to a rounded rect
        tw, th = 184, 108
        x, y = (w - tw) // 2, 8
        ...
        if pix and not pix.isNull():
            ...
            painter.fillRect(clip_rect,
                             QColor(0x35, 0x35, 0x35) if dark else QColor(0xe0, 0xe0, 0xe0))
        painter.restore()
        ...
        painter.drawText(8, name_y, w - 16, 20, Qt.AlignLeft | Qt.AlignVCenter,
                         self._info.name[:24])
        # Time
...
class AMELoaderPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._flow = FlowLayout(container, needAni=True)
    ...
    def _on_card_thumb(self, info):
        path, _ = QFileDialog.getOpenFileName(self, "选择封面", "",
                                                "Images (*.png *.jpg *.jpeg)")
        if path:
    ...
    def _on_import(self):
        path, _ = QFileDialog.getOpenFileName(self, "导入 JSON", "",
                                                "JSON (*.json)")
        if path:
    ...
    def _on_export(self):
        ...
        if path:
            ...
            InfoBar.success(title="已导出", content=f"工作流已导出",
                            orient=Qt.Horizontal, isClosable=True, duration=2000, parent=self)
